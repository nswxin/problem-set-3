...
data_scalar <- data * rep(betas, each = nrow(data))

error <- rnorm(1000, 0, 1)
data_scalar <- cbind(data_scalar, error)

data_scalar <- data_scalar %>%
  mutate(Y = rowSums(data_scalar)) 
...
mse <- map_dbl(1:16, get_mse)

train_mses <- as.data.frame(cbind(1:16, mse))
...
best_models <- map(1:16, model_cols)
...
mse_test <- map_dbl(1:16, get_mse_test)

test_mses <- as.data.frame(cbind(1:16, mse_test))
...
q7_outcomes <- map_dfr(1:16, q7_function)

q7 <- as.data.frame(cbind(1:16, q7_outcomes))

ggplot(q7, aes(`1:16`, sum_outcome)) +
  geom_line() +
  geom_point() +
  #geom_vline(xintercept = which.min(data_clean_cv_train$.estimate), linetype = 2) +
  #labs(title = "Subset selection",
